...
cat rhdm70-full.yaml | sed '/rhdmcentr-pvol/,+1 d' | sed '/claimName/d' | head -n -12
...
ssh root@host01 'for i in {1..200}; do cat rhdm70-full.yaml | sed "/rhdmcentr-pvol/,+1 d" | sed "/claimName/d" | head -n -12 > rhdm7-full-no-persistence.yaml && break || sleep 2; done'
...
ssh root@host01 'for i in {1..200}; do oc new-app --template=rhdm70-full-persistent -p APPLICATION_NAME="loan-demo" -p IMAGE_STREAM_NAMESPACE="openshift" -p KIE_ADMIN_USER="dmAdmin" -p KIE_ADMIN_PWD="redhatdm1!" -p KIE_SERVER_CONTROLLER_USER="kieserver" -p KIE_SERVER_CONTROLLER_PWD="kieserver1!" -p KIE_SERVER_USER="kieserver" -p KIE_SERVER_PWD="kieserver1!" -p MAVEN_REPO_USERNAME="dmAdmin" -p MAVEN_REPO_PASSWORD="redhatdm1!" -n loan-demo && break || sleep 2; done'
